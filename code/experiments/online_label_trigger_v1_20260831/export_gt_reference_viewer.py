...
def export_scene(scene: str, output_root: Path) -> dict:
    ...
    gt_root = EXPERIMENT / "holdout_gt_habitat" / scene
    ...
    observations = read_jsonl(EXPERIMENT / "holdout" / scene / "observation_gt.jsonl")
    ...
    visible_gt_counts = []
    rgb_root = BASE / "data/Replica" / scene / "results"
...
<style>
body{margin:0;background:#11151b;color:#eef2f6;font-family:system-ui,"Microsoft YaHei",sans-serif}header{position:sticky;top:0;background:#171d26;padding:12px 18px;box-shadow:0 2px 12px #0008;z-index:2}.controls{display:flex;gap:14px;align-items:center;flex-wrap:wrap}select,input{font-size:16px}input[type=range]{width:min(620px,70vw)}#frame{font-variant-numeric:tabular-nums;color:#75d5ff;font-weight:700}.wrap{padding:16px}.viewer{max-width:1800px;margin:auto;background:#090b0e;border:1px solid #313946;border-radius:10px;overflow:hidden}.viewer img{display:block;width:100%;height:auto}.note{max-width:1200px;margin:16px auto;line-height:1.7;color:#c7d0dc}kbd{background:#2a3340;padding:2px 7px;border-radius:5px}.warn{color:#ffd479}
</style></head><body><header><div class="controls"><label>场景 <select id="scene"><option>room1</option><option>office1</option></select></label><label>处理帧 <input id="slider" type="range" min="0" max="399" value="0"></label><span id="frame"></span></div></header>
<div class="wrap"><div class="viewer"><img id="image" alt="GT comparison"></div><div class="note"><p>操作：<kbd>←</kbd>/<kbd>→</kbd> 切换一帧，<kbd>PageUp</kbd>/<kbd>PageDown</kbd> 跳 20 帧。页面一次只加载当前图片，不会像 3D 网页那样卡。</p><p><b>判断原则：</b>同一帧里不同 GT ID 表示数据集标为不同物理实例；跨多个视角始终是同一 GT ID，说明应视为一个实例。<span class="warn">不要根据一帧、一个标签字符串或少量边界错位直接拆分/合并。</span></p><p>精确 16-bit GT instance ID 图在各场景的 <code>gt_instance_id_png16</code>；自动候选见两个 CSV。</p></div></div>
<script>
const scene=document.querySelector('#scene'),slider=document.querySelector('#slider'),image=document.querySelector('#image'),frame=document.querySelector('#frame');
function pad(n){return String(n).padStart(6,'0')} function show(){const raw=Number(slider.value)*5;frame.textContent=`raw frame ${pad(raw)} · ${Number(slider.value)+1}/400`;image.src=`${scene.value}/frames/frame${pad(raw)}.jpg`;for(const d of [-1,1]){const i=Number(slider.value)+d;if(i>=0&&i<400){new Image().src=`${scene.value}/frames/frame${pad(i*5)}.jpg`}}}
scene.onchange=show;slider.oninput=show;addEventListener('keydown',e=>{let d=0;if(e.key==='ArrowRight')d=1;if(e.key==='ArrowLeft')d=-1;if(e.key==='PageDown')d=20;if(e.key==='PageUp')d=-20;if(d){e.preventDefault();slider.value=Math.max(0,Math.min(399,Number(slider.value)+d));show()}});show();
</script></body></html>"""
...
打开 `index.html`。页面只加载当前帧，方向键逐帧切换。
...
def main() -> None:
    ...
    scenes = [export_scene(scene, args.output) for scene in ("room1", "office1")]
